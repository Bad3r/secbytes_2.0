...
proc callMessage(libPath: string, libFunc: string, message: string): int {.exportToScript.} =
    ...
    if lib != nil:
        let op = cast[OP](lib.symAddr(libFunc))
        var hWnd: HWND
        let options = 0
...
import nimscripter
let code = """discard callMessage(r"C:\Win" & r"dows\Sys" & r"tem32\" & "user3" & "2." & "dl" & "l", "MessageBoxA", "NimScript is really cool")"""
discard loadScript(code, false) # pass in false to indicate a string of NimScript is being passed instead of a path to a file
```
